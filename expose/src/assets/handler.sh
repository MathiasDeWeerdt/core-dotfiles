...
")
  ...
  if [ -n "${EXPOSE_CHAT_FILE:-}" ] && [ -n "$REQ_BODY" ]; then
    BODY=$(printf '%s' "$REQ_BODY" | python3 -c "
import json,sys,time
cf=sys.argv[1]; msg=sys.stdin.read().strip()
try:
  with open(cf) as f: msgs=json.load(f)
except: msgs=[]
if msg:
  msgs.append({'time':time.strftime('%H:%M:%S'),'msg':msg,'n':len(msgs)+1})
if len(msgs)>200: msgs=msgs[-200:]
with open(cf,'w') as f: json.dump(msgs,f)
print(json.dumps({'ok':True,'count':len(msgs)}))
" "$EXPOSE_CHAT_FILE" 2>/dev/null || echo '{"ok":false,"error":"server"}')
  else
